...
class CheckFindIdPwModel: NSObject {
    ...
    func downloadItems(code: String, id: String, name: String, birth: String) {
        ckCode = code
        if code == "findId" {
            urlPath = "http://localhost:8080/gambas/findId_query_ios.jsp?name=\(name)&birth=\(birth)"
        } else {
            urlPath = "http://localhost:8080/gambas/findPw_query_ios.jsp?id=\(id)&name=\(name)&birth=\(birth)"
        }
